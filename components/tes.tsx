...
export default function MyCalendar() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userRes.data.role === "Admin" || userRes.data.role === "C-Level") {
          fetchAllOutlets(token);
        } else {
          setSelectedOutlet(userRes.data.outlet.outletName);
          setOutletId(userRes.data.outlet.outletId);
        }
      } catch (err) {
    ...
    const fetchAllOutlets = async (token: string) => {
      try {
        ...
        if (data.length > 0) {
          setSelectedOutlet(data[0].outletName);
          setOutletId(data[0].outletId);
        }

        console.log("✅ All outlets:", data);
      } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="flex flex-col items-center">
        ...
        >
          {userRole === "Admin" || userRole === "C-Level" ? (
            <>
              <option value="">Pilih Outlet</option>
              {outlets.map((outlet, index) => (
                <option key={outlet.outletId} value={outlet.outletId}>
                  {outlet.outletName || `Outlet ${index}`}
                </option>
              ))}
            </>
          ) : (
            <option value={outletId || ""}>{selectedOutlet}</option>
          )}
        </select>
