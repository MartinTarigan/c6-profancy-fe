...
export default function EditMateriPelatihan() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const res = await fetch(`http://localhost:8080/api/training-materials/${id}`, {
        ...
        if (res.ok) {
          ...
          setToast({ type: "error", message: result.message || "Gagal mengambil data materi" });
        }
      } catch (err) {
        console.error(err);
        setToast({ type: "error", message: "Terjadi kesalahan saat memuat data" });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/training-materials/${id}/update`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        ...
        setToast({ type: "error", message: result.message || "Gagal memperbarui materi" });
      }
    } catch (err) {
      console.error(err);
      setToast({ type: "error", message: "Terjadi kesalahan saat mengirim data" });
    } finally {
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label htmlFor="title" className="block font-medium">Judul Materi</label>
                <input
              ...
              {/* Tipe */}
              <div className="space-y-2">
                <label htmlFor="type" className="block font-medium">Tipe Materi</label>
                <select
                  id="type" name="type"
              ...
              {/* Link */}
              <div className="space-y-2">
                <label htmlFor="link" className="block font-medium">Link Materi</label>
                <input
                  type="text" id="link" name="link"
                  value={formData.link} onChange={handleInputChange}
                  className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-primary"
                  placeholder="Link Material Video/Dokumen" required
                />
              </div>
            </div>
            ...
            <div className="space-y-6">
              {/* Deskripsi */}
              <div className="space-y-2">
                <label htmlFor="description" className="block font-medium">Deskripsi Materi</label>
                <textarea
                  id="description" name="description"
                  value={formData.description} onChange={handleInputChange}
                  className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-primary min-h-[150px]"
                  placeholder="Deskripsi materi pelatihan" required
                />
              </div>
              {/* Assign Roles */}
              <div className="space-y-2">
                <label htmlFor="assignedRoles" className="block font-medium">Assign User</label>
                <select
                  id="assignedRoles" name="assignedRoles" multiple
                  value={selectedRoles} onChange={handleRoleChange}
                  className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-primary"
                  size={4} required
                >
                ...
                <p className="text-xs text-gray-500">Hold Ctrl/Cmd untuk pilih beberapa</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mt-10">
            <Link href="/training/materi">
              <Button type="button" variant="outline" className="w-40 border-destructive text-destructive hover:bg-destructive/10">
                Batal
              </Button>
            </Link>
