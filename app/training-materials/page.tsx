  ...
  useEffect(() => {
    ...
    const fetchMaterials = async () => {
      try {
        ...
        if (storedToken) {
          const response = await fetch("http://localhost:8080/api/training-materials", {
            headers: {
    ...
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {materials.map((material) => (
          <div key={material.id} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex">
              <div className="w-1/3">
                <Image
                ...
                <div className="flex justify-end mb-2">
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${
                      material.type === "VIDEO" ? "bg-blue-100 text-blue-600" : "bg-indigo-100 text-indigo-600"
                    }`}
                  >
                    {material.type === "VIDEO" ? (
                      <>
