...
export default function TambahMateriPelatihan() {
  ...
  const validateForm = () => {
    if (!formData.title || !formData.link || !formData.description || formData.assignedRoles.length === 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setToast({ type: "error", message: "Authentication token not found. Please login again." });
      return;
    }
    ...
    try {
      ...
      const response = await fetch("https://sahabattens-tenscoffeeid.up.railway.app/api/training-materials/create", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        // Raise success toast
        setToast({ type: "success", message: "Materi pelatihan berhasil ditambahkan" });
        // After a short delay, navigate to the training materials page
        setTimeout(() => {
          router.push("/training-materials");
        }, 2000);
      } else {
        setToast({ type: "error", message: result.message || "Failed to add training material" });
      }
    } catch (err) {
      console.error("Error adding training material:", err);
      setToast({ type: "error", message: "An error occurred while adding training material" });
    } finally {
